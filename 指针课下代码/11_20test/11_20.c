...
int str_comp(char *p1, char *p2, int leng1, int leng2)
{
	int i,flag;
	if (leng1 != leng2)
	{
...
int main()
{
	char a[] = "abcde";
	char b[] = "bcdea";
	int result = str_comp(a, b, strlen(a), strlen(b));
	printf("%d", result);
	return 0;
}*/
/*void change(char a[], int len, int n)//字符串旋转
{
	int i, temp ;
	while (n > 0)
	{
...
int main()
{
	char a[] = "fhidwufiw";
	int time;
	printf("the start string is :>%s", a);
	printf("\n");
	printf("please input your time that you want to change:>\n");
	scanf("%d", &time);
	change(a, strlen(a), time);
